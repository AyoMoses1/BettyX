...
const useInputObj = () => {
  ...
  const nextAccountStart = localStorage.getItem('nextAccountStart');
  const prefix = localStorage.getItem('user_prefix');
  const user = localStorage.getItem('accountId');
  const isAgent = localStorage.getItem('user_role')
  ...
  const inputObjListForAgent = (register, handleChange, state, agentDetails,  errors) => [
    {
      name: 'numberOfAccounts',
      label: 'How many accounts to add?',
      type: 'number',
  ...
  if(isAgent === 'agent'){
    return {
      inputObjList: inputObjListForAgent
    }
  }
